...
if (Test-Path "angry-telegram" -PathType Container) {
    if (Test-Path (Join-Path "angry-telegram" "angry-telegram") -PathType Container) {
        Set-Location "angry-telegram"
    }
    python -m angry-telegram
    exit
}
...
$ret = Invoke-RestMethod -Uri "https://api.github.com/repos/git-for-windows/git/releases" -Headers @{'User-Agent'='angry-telegram installer'}
...
Invoke-WebRequest -Uri $download_url -OutFile (Join-Path $env:TEMP "git-scm-installer.exe") -Headers @{'User-Agent'='angry-telegram installer'; 'Accept'='application/octet-stream'}
...
git clone https://github.com/neongang/angry-telegram

Set-Location angry-telegram
python -m pip install -r requirements.txt
python -m angry-telegram
python -m angry-telegram --heroku # Stopgap
