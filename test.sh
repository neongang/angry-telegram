...
if [ ! -z "$TEST_BOT_TOKEN" ]; then
  ...
  python -m coverage run -m angry-telegram --phone "$TEST_BOT_TOKEN" --phone "$PHONE" --test-dc=22222 --no-web --self-test || exit $?
  python -m coverage xml --include 'angry-telegram/*'
else
